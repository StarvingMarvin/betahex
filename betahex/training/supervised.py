...
def make_train_model(feat):
    p = make_policy(feat, 64, [2, 3, -0.5, 2])
    ...
    def train_model(x, y, mode):
        ...
        white = x['white']
        empty = x['empty']
        white_edges = x['white_edges']

        if mode == learn.ModeKeys.TRAIN:

            tf.summary.image("probabilities_img", tf.reshape(probabilities, [-1, 19, 13, 1]), max_outputs=6)

            actually = tf.reshape(y, [-1, 19, 13, 1])
            ind = np.arange(19 * 13).reshape([1, 19, 13, 1])
            inds = np.repeat(ind, 64, axis=0)
            guess = tf.cast(tf.equal(inds, tf.reshape(first_guess, [64, 1, 1, 1])), tf.int8)
        ...
        if mode != learn.ModeKeys.INFER:
            ...
            penal = penalize_invalid(activation, empty, x['ones'], mode)
        ...
        if mode == learn.ModeKeys.TRAIN:
            train_op = tf.contrib.layers.optimize_loss(
                ...
                learning_rate=3e-3,
                optimizer="Adam",
                learning_rate_decay_fn=lambda lr, step: tf.train.exponential_decay(lr, step, 10000, .98)
            )
...
def main(unused_argv):
    ...
    est = learn.Estimator(
        model_fn=model_fn,
        model_dir="data/tf/models/64f-x2-relu-2-bn-3-bn-drop-5-2-mask-1e-4-penal-1-0-l3e-3d.98adam",
        config=config
    )
